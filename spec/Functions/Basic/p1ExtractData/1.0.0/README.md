# p1ExtractData

Filters a provided data structure according to a provided fields filter string and returns the filtered data structure.  
The function creates a new empty copy of the target data structure and then successively copies data from the input data structure into the copy.
It then returns this copy.

Note: the implementation follows the one of the generic p1FieldsFilter function.

## Overview

The fields-filtering string must comply with the NETCONF definitions.  
The filtering itself follows the same definitions.  

## Diagram

<p align="center">
  <img src="./p1ExtractData.png" alt="p1ExtractData" width="400" />
</p>

## Interface

Please find a detailed description of the [interface](./interface.yaml).

## Variables

Please find a detailed description of the [variables](./variables.yaml).

## NPM Module  

[mw-sdn-p1-fields-filter](https://www.npmjs.com/package/mw-sdn-p1-fields-filter)  